...
def main():
    ...
    if choice == "Home":
        ...
        user_input = st.text_input("You:", value="", key=f"user_input_{counter}",on_change=lambda: st.session_state.update(user_input=""))
    ...
    # Conversation History Menu
        ...
        # Ensure there is data after skipping the header
        if len(rows) > 1: 
            rows = rows[1:]  # Remove the header
            rows.reverse()  # Reverse to show latest first
